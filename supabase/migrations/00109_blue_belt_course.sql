-- Blue Belt course — schema + content (applied to DB via MCP migration + PostgREST upsert; this is the canonical record).
...
-- Lessons (markdown-formatted, verbatim wording from BB_STUDENT_COURSE.txt).
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$BB-ONB-01$TSSBB$, $TSSBB$bb_onboarding$TSSBB$, 501, $TSSBB$Blue Belt Value — Compromiso Consciente$TSSBB$, $TSSBB$Conscious Commitment$TSSBB$, $TSSBB$Mindset / Belt Value$TSSBB$, $TSSBB$Belt status: Blue Belt — entry
...
### How to internalize the shift

Suggested practice (one session):

## Read the full belt value aloud, twice.
## Reflect on your Yellow Belt sessions — were there moments where you wanted to skip a stage and you didn't? Were there moments where you did skip and you regretted it?
## Identify one specific temptation you have right now to skip a stage. Write it down. (Example: "I want to throw the snap without doing the projection first.")
## Set the intention for the next 3 sessions: every time I am tempted to skip a stage, I will say out loud: "There are no shortcuts. Walk the path."
## After each session, ask yourself: did I walk the path today?

That is Compromiso Consciente in practice.$TSSBB$, 15, $TSSBB$reading$TSSBB$, 1, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$BB-FOUND-01$TSSBB$, $TSSBB$blue_belt$TSSBB$, 500, $TSSBB$Blue Belt Foundation Sequence (17 elements)$TSSBB$, $TSSBB$Own these before the maneuvers$TSSBB$, $TSSBB$Foundation$TSSBB$, $TSSBB$**Goal:** Confirm that the 17 foundation elements from White + Yellow Belt are owned at Blue Belt level before you begin the new sequences.
...
### Two NEW Foundation principles at BB
...
### Duck Dive at BB level (10-step automatic chain)
...
## Read incoming wave (pre-stage decision)
## Paddle into the wave with controlled speed
## Grab rails at correct point ahead of the impact
## Compress hands + chest down onto board
## Submerge nose with body weight
## Drive the tail down with the foot (knee or foot push)
## Allow the wave's energy to pass over
## Lift the nose to surface as wave passes
## Repaddle immediately to maintain position
## Re-orient to the lineup
...
If you cannot tick all six boxes, return to that element for at least 3 sessions before continuing.$TSSBB$, 15, $TSSBB$reading$TSSBB$, 2, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-035$TSSBB$, $TSSBB$blue_belt$TSSBB$, 35, $TSSBB$Foot Position 1 (FP1) Operationalized at BB$TSSBB$, $TSSBB$Sequence #8 · Step 1$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
...
- I know when to switch into FP1 (before a maneuver) and when to switch out (after)$TSSBB$, 15, $TSSBB$reading$TSSBB$, 3, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-036$TSSBB$, $TSSBB$blue_belt$TSSBB$, 36, $TSSBB$Pump Frontside$TSSBB$, $TSSBB$Sequence #8 · Step 2$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
...
- I am in FP3 throughout the pump$TSSBB$, 15, $TSSBB$reading$TSSBB$, 4, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-037$TSSBB$, $TSSBB$blue_belt$TSSBB$, 37, $TSSBB$Pump Backside$TSSBB$, $TSSBB$Sequence #9 · Step 1$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
**Block:** 5 + 6
...
- ERR-114 — Same FS body mechanics on BS — does not translate$TSSBB$, 15, $TSSBB$reading$TSSBB$, 5, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-038$TSSBB$, $TSSBB$blue_belt$TSSBB$, 38, $TSSBB$BS Body Mechanics$TSSBB$, $TSSBB$Sequence #9 · Step 2$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
**Block:** 5
...
- I feel my oblique engage during BS rotation$TSSBB$, 15, $TSSBB$reading$TSSBB$, 6, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-039$TSSBB$, $TSSBB$blue_belt$TSSBB$, 39, $TSSBB$Bottom Turn Medium$TSSBB$, $TSSBB$Sequence #10 · Step 1$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
...
- My BT flows directly into Projection without a pause$TSSBB$, 15, $TSSBB$reading$TSSBB$, 7, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-040$TSSBB$, $TSSBB$blue_belt$TSSBB$, 40, $TSSBB$Projection$TSSBB$, $TSSBB$Sequence #10 · Step 2$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
**Block:** 6 (Projection)
...
- My arms rise to lead the body upward$TSSBB$, 15, $TSSBB$reading$TSSBB$, 8, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-041$TSSBB$, $TSSBB$blue_belt$TSSBB$, 41, $TSSBB$Cruz Snap$TSSBB$, $TSSBB$Sequence #10 · Step 3$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
**Block:** 7 (Maneuver)
...
- My hand crosses early enough to lead the rail change$TSSBB$, 15, $TSSBB$reading$TSSBB$, 9, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-042$TSSBB$, $TSSBB$blue_belt$TSSBB$, 42, $TSSBB$Grenade$TSSBB$, $TSSBB$Sequence #10 · Step 4$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
...
- I am ready for the next cycle immediately$TSSBB$, 15, $TSSBB$reading$TSSBB$, 10, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-043$TSSBB$, $TSSBB$blue_belt$TSSBB$, 43, $TSSBB$Choke (BS Projection)$TSSBB$, $TSSBB$Sequence #11 · Step 1$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
...
- My weight stays forward (does not drift backward)$TSSBB$, 15, $TSSBB$reading$TSSBB$, 11, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-044$TSSBB$, $TSSBB$blue_belt$TSSBB$, 44, $TSSBB$Tapaloco Snap$TSSBB$, $TSSBB$Sequence #11 · Step 2$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
**Block:** 7 (Maneuver)
...
## With ARM (least exaggerated)
## With ELBOW (intermediate)
## With SHOULDER (more exaggerated)
## With HAND, palm UP, over head, covering ear (CANONICAL IDEAL)
...
- My weight stayed forward throughout$TSSBB$, 15, $TSSBB$reading$TSSBB$, 12, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-045$TSSBB$, $TSSBB$blue_belt$TSSBB$, 45, $TSSBB$Elbow (BS Closure)$TSSBB$, $TSSBB$Sequence #11 · Step 3$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
**Block:** 7 (Maneuver / Expression — closure)
...
- I see water thrown at the end of the maneuver$TSSBB$, 15, $TSSBB$reading$TSSBB$, 13, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-046$TSSBB$, $TSSBB$blue_belt$TSSBB$, 46, $TSSBB$Cruz Cutback$TSSBB$, $TSSBB$Sequence #12 · Step 1$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
**Block:** 7 (Maneuver)
...
- My Cruz position stayed engaged until I saw the pocket$TSSBB$, 15, $TSSBB$reading$TSSBB$, 14, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-047$TSSBB$, $TSSBB$blue_belt$TSSBB$, 47, $TSSBB$Hold$TSSBB$, $TSSBB$Sequence #12 · Step 2$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
...
- All my Cruz details (palms, chest, eyes, oblique) stayed engaged during the Hold$TSSBB$, 15, $TSSBB$reading$TSSBB$, 15, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-048$TSSBB$, $TSSBB$blue_belt$TSSBB$, 48, $TSSBB$Tapaloco Cutback$TSSBB$, $TSSBB$Sequence #13 · Step 1$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
**Block:** 7 (Maneuver)
...
- My cutback drew a single continuous line (not two disconnected movements)$TSSBB$, 15, $TSSBB$reading$TSSBB$, 16, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$STP-049$TSSBB$, $TSSBB$blue_belt$TSSBB$, 49, $TSSBB$Hold Rotation$TSSBB$, $TSSBB$Sequence #13 · Step 2$TSSBB$, $TSSBB$Technical$TSSBB$, $TSSBB$**Pillar:** Technical
**Block:** 7 (Maneuver · integrated within)
...
- My weight stayed forward$TSSBB$, 15, $TSSBB$reading$TSSBB$, 17, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$BB-CONCEPTS-01$TSSBB$, $TSSBB$blue_belt$TSSBB$, 590, $TSSBB$The Four Blue Belt Concepts$TSSBB$, $TSSBB$Floater · Impulso · Low Finish · BT Concept$TSSBB$, $TSSBB$Technical / Concepts$TSSBB$, $TSSBB$These are CONCEPTS (not Sequences) — transversal techniques you carry across multiple sequences at BB level and beyond. They are not bound to a single STP — they apply everywhere.
...
Internalize these four concepts. They are your operational toolkit beyond the Universal Formula.$TSSBB$, 15, $TSSBB$reading$TSSBB$, 18, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$BB-MOD-INT$TSSBB$, $TSSBB$blue_belt$TSSBB$, 600, $TSSBB$The Complete Blue Belt Ride (Integration Module)$TSSBB$, $TSSBB$The unified Infinite Circle$TSSBB$, $TSSBB$Integration Module$TSSBB$, $TSSBB$You now have everything you need. This section integrates all your BB STPs + Concepts into the Infinite Circle — the continuous execution loop within the wave.

### What the Infinite Circle is
...
### The Complete BB Operational Chain (15 stages)
...
In every BB session:
## Read the chain before getting in the water
## Execute each stage with awareness
## After the session, audit which stage was strongest / weakest
## Pick ONE stage to drill in the next session
...
## Is it an execution or decision problem?
## What is causing it? (Physical / Mental / Technical / Tactical)
...
That is the practice. Do it every session. It is what separates the BB student from the YB student.$TSSBB$, 15, $TSSBB$reading$TSSBB$, 19, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
insert into lessons (id, course_section, step_number, title, subtitle, pillar, description_md, estimated_minutes, lesson_type, display_order, active, status_v1) values ($TSSBB$BB-EXIT-01$TSSBB$, $TSSBB$blue_belt$TSSBB$, 800, $TSSBB$Blue Belt Exit Test (Self-Evaluation)$TSSBB$, $TSSBB$Self-evaluation$TSSBB$, $TSSBB$Evaluation$TSSBB$, $TSSBB$The Blue Belt Exit Test certifies that you have owned the Universal Sequence Formula across the 6 BB sequences and have internalized Compromiso Consciente.
...
### The 6 Sequences · 5 Criteria · Self-Evaluation Rubric
...
### Belt Value Self-Evaluation (Compromiso Consciente)
...
## Weight drifts backward consistently → loses speed, gets stuck on wave
## Cutback in 2 movements → Hold failure · breaks the connected line
## Skipping a Universal Formula stage → breaks the structural commitment

Drill the corresponding sequence in isolation until these stop.

### Acceptable / Ideal / Unacceptable Rubric
...
### Minimum sessions before Exit Test
...
> There are no shortcuts. You have walked the path.$TSSBB$, 15, $TSSBB$reading$TSSBB$, 20, true, $TSSBB$PRODUCTIZED$TSSBB$) on conflict (id) do update set title=excluded.title, subtitle=excluded.subtitle, pillar=excluded.pillar, description_md=excluded.description_md, course_section=excluded.course_section, step_number=excluded.step_number, display_order=excluded.display_order;
