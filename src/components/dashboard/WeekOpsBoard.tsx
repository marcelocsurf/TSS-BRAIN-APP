...
export async function WeekOpsBoard({ academyId }: { academyId: string }) {
  ...
  try {
    ...
    return (
      <div className="mb-6 rounded-lg bg-[#F7F9FA] border border-amber-200 p-4">
  ...
  return (
    <div className="mb-6 rounded-lg bg-[#F7F9FA] border border-[#DCD7C6] shadow-sm p-4">
      <div className="flex items-center justify-between flex-wrap gap-2 mb-1">
        ...
        <p className="text-[11px] text-[#55666E]">Lo que los coaches fijaron en su Vista Semana · 📋 copia el día para el chat de performance</p>
      </div>
      ...
      <div className="space-y-3 mt-2">
        {days.map((d) => {
          ...
          return (
            <div key={d} className="rounded-[5px] border overflow-hidden" style={{ borderColor: isToday ? '#00D2FF' : '#F0F2F4', borderLeftWidth: isToday ? 6 : 1, borderLeftColor: isToday ? '#00D2FF' : '#F0F2F4' }}>
              <div className="flex items-center justify-between px-3 py-2" style={{ background: isToday ? '#061C2B' : '#FAFBFC' }}>
                <p className="text-[11px] font-extrabold uppercase" style={{ color: isToday ? '#FFFFFF' : '#061C2B' }}>
                  ...
                  <span className="text-[#55666E] font-semibold normal-case"> · {rows.length} servicio{rows.length === 1 ? '' : 's'}</span>
                </p>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-[11px]">
                  <thead>
                    <tr className="text-left text-[#55666E] border-b border-[#DCD7C6]">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {rows.map((r, i) => (
                      <tr key={`${r.campId}-${i}`} className="align-top hover:bg-[#F7F9FA]">
                        ...
                        <td className="py-2 pr-2">
                          ...
                          {r.staff.length > 0 && <span className="text-[#55666E]"> · {r.staff.join(' · ')}</span>}
                        </td>
                        <td className="py-2 pr-2 font-extrabold whitespace-nowrap" style={{ color: '#0090B0' }}>{r.meeting ?? '—'}</td>
                        <td className="py-2 pr-2 whitespace-nowrap text-[#55666E]">
                        ...
                        <td className="py-2 pr-2 text-[#55666E]">{r.venue ?? '—'}</td>
                        <td className="py-2 pr-2 text-[#55666E]">{r.spaces.length ? r.spaces.join(' · ') : '—'}</td>
                        <td className="py-2 pr-3 text-[#55666E] whitespace-nowrap">
