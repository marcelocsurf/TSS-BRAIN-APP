...
export function Step12MissionTime({ formState, options, onSelect }: Props) {
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-[#1A1A2E]">Mission Time</h3>
      <div className="grid grid-cols-3 gap-2">
        {options.map((opt) => {
          const isSelected = formState.mission_time === opt.value;
          return (
            <button
              key={opt.id}
              type="button"
