...
export function Step09WarmUp({ formState, options, onSelect }: Props) {
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-[#1A1A2E]">Warm-up</h3>
      <div className="space-y-2">
        {options.map((opt) => {
          const isSelected = formState.warm_up === opt.value;
          return (
            <button
              key={opt.id}
              type="button"
