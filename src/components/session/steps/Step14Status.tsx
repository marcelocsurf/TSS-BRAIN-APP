...
const STATUS_OPTIONS: { value: SessionStatus; label: string; color: string; bg: string }[] = [
  { value: 'not_yet',      label: 'Not Yet',       color: 'text-[#55666E]',  bg: 'bg-[#F7F9FA] border-[#DCD7C6]' },
...
export function Step14Status({ formState, onSelect }: Props) {
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-[#55666E]">How did the student perform?</p>
        ...
        {STATUS_OPTIONS.map((opt) => {
          ...
          return (
            <button
              ...
              className={`w-full p-4 rounded-[5px] border-2 text-left text-sm font-semibold transition-all ${
                isSelected
                  ? `${opt.bg} ${opt.color}`
