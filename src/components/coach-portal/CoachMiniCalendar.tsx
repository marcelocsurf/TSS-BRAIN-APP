...
export function CoachMiniCalendar({ services, onOpen }: {
  services: any[];
    ...
    <div className="rounded-2xl border border-white/10 p-3" style={{ background: '#0F1E33' }}>
        ...
        {cells.map((d) => {
          ...
          return (
            <button
              ...
              style={{
                background: isSel ? '#5AC3E7' : has ? 'rgba(90,195,231,.12)' : 'transparent',
                color: isSel ? '#0A1628' : inMonth ? '#e5eef5' : '#3a4a5e',
                border: isToday && !isSel ? '1px solid #5AC3E7' : '1px solid transparent',
              }}
            >
              {d.getDate()}
              {has && <span className="w-1 h-1 rounded-full mt-0.5" style={{ background: isSel ? '#0A1628' : '#5AC3E7' }} />}
            </button>
          );
        })}
      </div>
      ...
      <div className="mt-3 border-t border-white/10 pt-2 space-y-1.5">
        ...
        {selectedServices.length === 0 ? (
          <p className="text-[11px] text-white/40">No services this day.</p>
