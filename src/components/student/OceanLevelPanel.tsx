...
export function OceanLevelPanel({ studentId, coachId, currentLevel, history, provisional }: Props) {
  ...
  const levelColor: Record<string, string> = {
    beginner: 'bg-[#EDF3F5] text-[#10263B]',
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          <span className={`text-xs px-2 py-0.5 rounded-full ${levelColor[currentLevel || ''] || 'bg-[#F7F9FA] text-[#55666E]'}`}>
      ...
      {currentInfo && (
        <p className="text-[11px] text-[#55666E] italic">{currentInfo.cleared}</p>
      )}

      {/* Inline form */}
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-[#F7F9FA] rounded-lg p-3 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] text-[#55666E] mb-1">New Level</label>
              <select
                value={form.newLevel}
            ...
            <div>
              <label className="block text-[10px] text-[#55666E] mb-1">Method</label>
              <select
                value={form.method}
          ...
          <div>
            <label className="block text-[10px] text-[#55666E] mb-1">Notes (optional)</label>
            <textarea
              value={form.notes}
              onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
              rows={2}
              className="w-full px-2 py-1.5 border border-[#DCD7C6] rounded text-sm resize-none"
      ...
      {/* History */}
      {history.length > 0 && (
        <div className="space-y-2 pt-2 border-t border-[#DCD7C6]">
          <p className="text-[10px] font-semibold text-[#55666E] uppercase">Level Change History</p>
          {history.slice(0, 5).map((ev: any) => (
            <div key={ev.id} className="flex items-start justify-between gap-2 text-xs">
              <div className="min-w-0">
                <p className="text-[#10263B]">
                ...
                {ev.notes && <p className="text-[#55666E] mt-0.5 truncate">{ev.notes}</p>}
              </div>
            </div>
          ))}
        </div>
