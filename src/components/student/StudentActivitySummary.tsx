  ...
  if (!d) {
    return (
      <div className="rounded-lg bg-[#F7F9FA] border border-[#DCD7C6] p-4">
        <p className="text-xs text-[#55666E]">Cargando actividad del alumno…</p>
      </div>
    );
  }
    ...
    <div className="rounded-lg bg-[#F7F9FA] border border-[#DCD7C6] p-4 space-y-3" style={{ borderLeft: '4px solid #00A8CC' }}>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
        {[
          ...
          <div key={x.l} className="rounded-[5px] py-2.5 px-2 text-center" style={{ background: '#F8FAFC', border: '1px solid #EEF2F6' }}>
            <p className="text-[14px] font-extrabold leading-tight" style={{ color: x.accent }}>{x.n}</p>
            <p className="text-[9.5px] font-mono uppercase tracking-wide text-[#55666E] mt-0.5">{x.l}</p>
          </div>
        ))}
      </div>
