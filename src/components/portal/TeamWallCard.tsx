...
const INK = '#061C2B', CYAN = '#00D2FF', GOLD = '#B8C7D1';
    ...
    <div className="rounded-lg p-4" style={{ background: '#0A2532', border: '1px solid rgba(0,210,255,.25)' }}>
      <button type="button" onClick={() => setOpen(!open)} className="w-full text-left">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px]" style={{ ...MONO, color: '#00D2FF' }}>{open ? '▴' : `${data.wall.length}`}</span>
        </div>
        {!open && latest && (
        ...
        <div className="mt-3 space-y-2">
          <div className="flex gap-2">
            <input value={body} onChange={(e) => setBody(e.target.value)} placeholder="Message your team…"
              className="flex-1 rounded-lg px-3 py-2 text-[12px]" style={{ background: 'rgba(255,255,255,.07)', border: '1px solid rgba(255,255,255,.14)', color: '#eaf4fa' }} />
            <button type="button" disabled={busy || !body.trim()} onClick={post}
              className="rounded-[5px] px-4 text-[10px] font-bold uppercase tracking-wider disabled:opacity-40" style={{ ...MONO, background: CYAN, color: INK }}>
              Send
            </button>
          </div>
          ...
          <div className="space-y-1.5 max-h-72 overflow-y-auto">
            {data.wall.map((w) => (
              <div key={w.id} className="rounded-lg px-3 py-2" style={{ background: w.mine ? 'rgba(0,210,255,.08)' : 'rgba(255,255,255,.05)', borderLeft: `3px solid ${w.mine ? CYAN : GOLD}` }}>
