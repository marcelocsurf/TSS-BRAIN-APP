...
export async function CampCostPanel({ campInstanceId }: { campInstanceId: string }) {
  ...
  const mixChips: Array<[string, number, string]> = [
    ['Full', b.saleMix.full, 'bg-[#EDF3F5] text-[#55666E] border-[#DCD7C6]'],
  ...
  return (
    <div className="bg-[#E9E2D2] rounded-lg border border-[#DCD7C6] shadow-sm p-4">
      <div className="flex items-center justify-between gap-2 mb-1">
        ...
        <Link href="/costs" className="text-[10px] font-mono uppercase tracking-wider text-[#55666E] hover:text-[var(--tss-navy)]">Rates →</Link>
      </div>
      <p className="text-[10px] text-[#55666E] mb-3" style={{ fontFamily: 'var(--font-plex), IBM Plex Mono, monospace' }}>
      ...
      <div className="flex items-center justify-end gap-4 text-[9px] font-mono uppercase tracking-wider text-[#55666E] pb-1">
      ...
      <div className="divide-y divide-gray-50">
        {b.lines.map((l, i) => (
          <div key={i} className="flex items-center justify-between gap-2 py-1.5">
            <p className="text-[12px] text-[#10263B] min-w-0 truncate">
              {l.name} <span className="text-[#55666E]">· {money(l.unit_cents)} × {l.qty_label}</span>
            </p>
            <div className="flex items-center gap-4 shrink-0">
              <p className="w-14 text-right text-[12px] font-semibold text-[var(--tss-navy)]">{money(l.total_cents)}</p>
              <p className={`w-14 text-right text-[12px] ${started ? 'font-semibold text-[#10263B]' : 'text-[#B8B1A0]'}`}>{money(l.real_total_cents)}</p>
            </div>
          </div>
        ))}
      </div>

      <div className="mt-2 pt-2 border-t border-[#DCD7C6] space-y-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <p className={`w-14 text-right text-[14px] font-bold ${started ? 'text-[#10263B]' : 'text-[#B8B1A0]'}`}>{money(b.realTotalCents)}</p>
          </div>
        </div>
        <div className="flex items-center justify-between">
          <p className="text-[12px] text-[#55666E]">Revenue committed <span className="text-[#55666E]">({money(b.revenueCollectedCents)} collected)</span></p>
        ...
        {started && (
          <div className="flex items-center justify-between px-2.5">
            <p className="text-[11px] text-[#55666E]">Margin real (to date)</p>
