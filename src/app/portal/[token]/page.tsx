...
export default async function StudentPortalPage({ params, searchParams }: Props) {
  ...
  const [materials, drills, drillsMissions, pendingSurveys, submittedSurveys, courseCatalog, myCoach, pendingExperience,
    ...
    getPendingSurveys(token).catch(() => [] as any[]),
    getSubmittedSurveys(token).catch(() => [] as any[]),
    getCourseCatalog(token),
    coachUnlocked ? getMyCoachData(token).catch(() => null) : Promise.resolve(null),
    ...
    // The Lineup y el estado del foco del coach: EN PARALELO con todo lo
  ...
  const validTabs = ['home', 'course', 'sequence', 'lineup', 'sessions', 'feedback', 'glossary', 'my-coach'];
    ...
    <>
      ...
      <PortalTabs
        data={{
          ...
          // Una cinta CONFIRMADA por un coach ya no necesita quiz (Androide, Purple,
          // veía la tarjeta): se marca como 'hecho' para que el Home no la ofrezca.
