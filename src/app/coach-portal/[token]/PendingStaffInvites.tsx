...
function InviteCard({ invite }: { invite: StaffInvite }) {
  ...
  if (done) {
    return (
      <div className={`rounded-lg border p-4 ${done === 'accepted' ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'}`}>
  ...
  return (
    <div className="rounded-lg bg-[#E9E2D2] border border-[#DCD7C6] border border-[#DCD7C6] shadow-sm p-4" style={{ borderLeft: '4px solid #FFD166' }}>
      <div className="flex items-start gap-2.5 mb-3">
        <HandHelping size={18} className="shrink-0 mt-0.5" style={{ color: '#B8860B' }} />
        <div className="min-w-0">
          <p className="text-[11px] font-mono uppercase tracking-[0.14em]" style={{ color: '#B8860B' }}>
      ...
      {mode === 'rejecting' ? (
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => { setMode('idle'); setError(''); }} className="px-3 py-2.5 text-sm text-[#55666E]">
              Back
            </button>
          </div>
        </div>
