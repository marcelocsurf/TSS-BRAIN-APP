...
import { AuthorCard } from '@/components/coach-portal/AuthorCard';
  ...
  return (
    ...
    >
          ...
          <div className="rounded-lg p-3 space-y-4" style={{ background: isSupport ? '#000' : 'transparent' }}>
            ...
            <div className="flex gap-2">
              ...
              <a href={`/manual/index.html?role=${isSupport ? 'support' : 'coach'}`} target="_blank" rel="noreferrer"
                className="flex-1 text-center text-[11px] font-mono uppercase tracking-[0.14em] font-semibold px-3 py-2.5 rounded-full border border-[var(--tss-cyan,#00D2FF)]/40 text-[var(--tss-cyan,#00D2FF)] hover:bg-[var(--tss-cyan,#00D2FF)]/10">
                📘 Cómo usar el app
              </a>
              {canSell && (
      ...
      <div className={`text-center py-4 ${plannerOpen ? 'hidden' : 'pb-24'}`}>
        <p className="text-[11px] text-white/30">The Surf Sequence -- {BRAND.tagline}</p>
      </div>
    </div>
  );
}
...
function EmRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-2 text-sm">
      <span className="text-[11px] font-mono uppercase tracking-wider text-white/40 w-20 shrink-0 pt-0.5">{label}</span>
...
function EmRowLight({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-2 text-sm">
      <span className="text-[11px] font-mono uppercase tracking-wider text-[#55666E] w-20 shrink-0 pt-0.5">{label}</span>
      <span className="text-[#10263B] flex-1 whitespace-pre-line">{value}</span>
    </div>
  );
}
    ...
    <div className="rounded-lg p-3 space-y-4" style={{ background: '#000' }}>
      ...
      <div className="rounded-lg border border-white/10 p-4" style={{ background: '#0A2532' }}>
        <div className="flex items-center justify-between mb-2">
          <p className="text-[11px] font-mono uppercase tracking-wider text-[var(--tss-cyan,#00D2FF)]">Sales goal</p>
          <p className="text-[11px] text-white/60">{goalPct}% of {money(metaCents)}</p>
        </div>
        <div className="h-2.5 rounded-full overflow-hidden bg-[#F7F9FA]/10 mb-1">
          <div style={{ width: `${goalPct}%`, background: '#00D2FF', height: '100%' }} />
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 mt-3">
          {[
            ...
            <div key={x.l} className="rounded-[5px] bg-[#F7F9FA]/[0.04] px-2 py-2 text-center">
              <p className="text-lg font-bold text-white leading-none">{x.v}</p>
              <p className="text-[11px] uppercase tracking-wider text-white/40 mt-1" style={{ fontFamily: 'DM Mono, monospace' }}>{x.l}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-lg border border-white/10 p-4" style={{ background: '#0A2532' }}>
              ...
              return (
                <div key={s.id} className="rounded-lg bg-[#F7F9FA]/[0.04] px-3 py-2.5">
                  <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="mt-1.5 flex flex-wrap gap-1.5">
                        {s.sales_deck_url && (
                          <a href={s.sales_deck_url} target="_blank" rel="noreferrer"
                            className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-1 rounded-full border border-[#00D2FF]/50 text-[#00D2FF] hover:bg-[#00D2FF]/10">
                            📽 Selling deck →
                          </a>
                        )}
                        {!full && (
                          <button onClick={() => setReservingId(reservingId === s.id ? null : s.id)}
                            className={`inline-flex items-center gap-1 text-[11px] font-bold px-2 py-1 rounded-full ${reservingId === s.id ? 'bg-[#F7F9FA]/15 text-white' : 'text-[#0A2532]'}`}
                    ...
                    <span className={`text-[11px] font-bold px-2 py-1 rounded-full shrink-0 ${full ? 'bg-[#F7F9FA]/10 text-white/50' : 'text-[#0A2532]'}`} style={!full ? { background: '#00D2FF' } : undefined}>
        ...
        <p className="text-[11px] text-white/30 mt-3">You reserve, the coordinator confirms the payment.</p>
      </div>

      {/* 2D — My sales (form component below) */}
      <div className="rounded-lg border border-white/10 p-4" style={{ background: '#0A2532' }}>
        <p className="text-[11px] font-mono uppercase tracking-wider text-[var(--tss-cyan,#00D2FF)] mb-3">
        ...
        {!sales ? (
          ...
          <div className="space-y-1.5">
            {sales.map((v) => (
              <div key={v.id} className="rounded-lg bg-[#F7F9FA]/[0.04] px-3 py-2 flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-[13px] text-white font-medium truncate">{v.student_name}</p>
                  <p className="text-[11px] text-white/40 truncate" style={{ fontFamily: 'DM Mono, monospace' }}>
                    {v.camp_name}{v.start_date ? ` · ${new Date(v.start_date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}` : ''}
                  </p>
                </div>
                <span className={`text-[10px] font-bold px-2 py-1 rounded-full shrink-0 ${v.enrollment_status !== 'active' ? 'bg-[#F7F9FA]/10 text-white/40' : v.payment_status === 'paid' ? 'bg-[#06D6A0]/20 text-[#06D6A0]' : 'bg-[#FFD166]/15 text-[#FFD166]'}`}>
...
function ReserveForm({ token, campId, onDone }: { token: string; campId: string; onDone: () => void }) {
  ...
  const inp = 'w-full text-[13px] px-3 py-2 rounded-lg bg-white/[0.06] border border-white/15 text-white placeholder-white/30 focus:outline-none focus:border-[#00D2FF]';
  return (
    <div className="mt-2.5 rounded-[5px] border border-white/10 bg-[#F7F9FA]/[0.03] p-3 space-y-2">
      <div className="flex gap-1.5">
        {(['search', 'new'] as const).map((m) => (
          <button key={m} onClick={() => { setMode(m); setPicked(null); setMsg(null); }}
            className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${mode === m ? 'text-[#0A2532]' : 'text-white/50 border border-white/15'}`}
            style={mode === m ? { background: '#00D2FF' } : undefined}>
      ...
      {mode === 'search' ? (
        picked ? (
          <div className="flex items-center justify-between rounded-lg bg-[#00D2FF]/10 border border-[#00D2FF]/40 px-3 py-2">
          ...
          <>
            ...
            {results.length > 0 && (
              <div className="rounded-lg overflow-hidden border border-white/10 divide-y divide-white/5">
                {results.map((r) => (
                  <button key={r.id} onClick={() => setPicked(r)} className="w-full text-left px-3 py-2 bg-[#F7F9FA]/[0.04] hover:bg-[#F7F9FA]/[0.09]">
                    <p className="text-[13px] text-white">{r.name}</p>
                    {r.email && <p className="text-[11px] text-white/40">{r.email}</p>}
                  </button>
                ))}
              </div>
            )}
      ...
      <button onClick={submit} disabled={busy || (mode === 'search' ? !picked : !first.trim())}
        className="w-full py-2.5 rounded-full text-[12px] font-bold text-[#0A2532] disabled:opacity-40"
...
function HomeTab({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg overflow-hidden" style={{ background: '#061C2B' }}>
        <div className="px-5 pt-5 pb-6">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <p className="text-[11px]" style={{ ...F_LABEL, color: '#00D2FF' }}>Here and now · Coach portal</p>
          ...
          <div className="mt-4 flex items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <span className="mt-2 inline-flex items-center rounded-full px-2.5 py-1 text-[11px]" style={{ ...F_LABEL, background: 'rgba(0,210,255,.15)', color: '#00D2FF' }}>
          ...
          {/* Certification card */}
          <div className="mt-5 rounded-lg p-4" style={{ background: 'rgba(255,255,255,.06)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.1)' }}>
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="inline-flex items-center gap-1.5 shrink-0 text-[11px]" style={{ ...F_LABEL, color: 'rgba(247,249,250,.7)' }}>
            ...
            <p className="mt-2 text-[11px]" style={{ ...F_LABEL, color: 'rgba(247,249,250,.55)' }}>
      ...
      {todayLogistics && (
        <div className="rounded-lg bg-[#E9E2D2] border border-[#DCD7C6] p-5" style={{ boxShadow: '0 10px 30px rgba(6,28,43,.06), inset 0 0 0 1px rgba(6,28,43,.05)' }}>
          <p className="text-[11px]" style={{ ...F_LABEL, color: '#00A8CC' }}>
          ...
          <div className="mt-4 space-y-2.5">
            ...
            <div className="flex items-center gap-2.5 text-[11px]" style={{ ...F_LABEL, color: '#55666E' }}>
      ...
      <div className="grid grid-cols-4 gap-2.5">
        {[
          ...
          <button
            ...
            onClick={t.onClick}
            disabled={!t.onClick}
            className="rounded-lg bg-[#E9E2D2] border border-[#DCD7C6] px-2 py-3 text-center disabled:cursor-default"
          ...
          >
            ...
            <p className="mt-1.5 text-[11px] leading-tight" style={{ ...F_LABEL, letterSpacing: '0.12em', color: '#55666E' }}>{t.label}</p>
          </button>
        ))}
      </div>
      ...
      {totalLessons > 0 && (
        <div className="rounded-lg bg-[#E9E2D2] border border-[#DCD7C6] p-4" style={{ boxShadow: 'inset 0 0 0 1px rgba(6,28,43,.05)' }}>
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[11px]" style={{ ...F_LABEL, color: '#55666E' }}>Your course progress</span>
      ...
      {profileIncomplete && (
        <Link
          href={`/coach-portal/${coach.portal_token}/profile`}
          className="block rounded-lg p-4 bg-[rgba(255,209,102,.25)]"
          style={{ boxShadow: 'inset 0 0 0 1px rgba(217,119,6,.25)' }}
        >
          <p className="text-sm font-semibold text-[#10263B]">Complete your profile</p>
          <p className="text-xs text-[#10263B] mt-0.5 leading-relaxed">
            Emergency contact, medical info and waiver. Required before teaching official sessions.
          </p>
          <p className="text-[11px] mt-2" style={{ ...F_LABEL, color: '#10263B' }}>Fill it now →</p>
        </Link>
      )}

      {/* ── Your next classes ── */}
      {upcoming.length > 0 && (
        <details open className="rounded-lg overflow-hidden bg-[#E9E2D2] border border-[#DCD7C6]" style={{ boxShadow: 'inset 0 0 0 1px rgba(6,28,43,.05)' }}>
          <summary className="cursor-pointer list-none px-4 py-3 flex items-center justify-between">
            <span className="inline-flex items-center gap-2 text-sm font-semibold" style={{ color: '#061C2B' }}>
              ...
              Your next classes
              <span className="text-[11px] px-2 py-0.5 rounded-full" style={{ background: 'rgba(0,210,255,.12)', color: '#00A8CC' }}>{upcoming.length}</span>
            </span>
            <ChevronDown size={16} style={{ color: '#55666E' }} />
          </summary>
              ...
              return (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] mt-0.5" style={{ ...F_LABEL, letterSpacing: '0.12em', color: '#55666E' }}>
                  ...
                  <ChevronRight size={15} className="shrink-0" style={{ color: '#55666E' }} />
                </button>
              );
            })}
          </div>
      ...
      <AuthorCard token={coach.portal_token} />
...
function CoursesTab({
              ...
              <>
                ...
                {detail.videos.length > 0 && (
                  <div className="space-y-2">
                    {detail.videos.map((v) => (
                      <div key={v.id} className="bg-black rounded-lg overflow-hidden aspect-video">
                        <iframe
            ...
            {!detail.lesson.coach_what_md && detail.lesson.lesson_type === 'test' && detail.quizzes.length > 0 ? (
              ...
              (!detail.lesson.coach_what_md || detail.quizzes.length === 0) && (
                <button
                  ...
                  className={`w-full py-3 text-sm font-semibold rounded-[5px] transition-all ${
                    isCompleted
  ...
  const renderCard = (c: any) => {
    ...
    const accent = isValue ? '#EC4899' : isMethod ? '#00D2FF' : isYb ? '#FFD166' : isBb ? '#3B82F6' : '#94A3B8';
    return (
      <button
        key={c.id}
        type="button"
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0 flex-1 flex items-center gap-2.5">
            {isCompleted ? (
              ...
              <Lock size={15} strokeWidth={2} className="shrink-0 text-[#55666E]" />
            ) : (
              <span className="shrink-0 w-[15px] h-[15px] rounded-full border-2" style={{ borderColor: isInProgress ? '#00D2FF' : '#DCD7C6', background: isInProgress ? 'rgba(0,210,255,.25)' : 'transparent' }} />
            )}
            <span className="min-w-0">
              <span className="block text-[11px]" style={{ ...F_LABEL, color: '#00A8CC' }}>
                {c.id} · ~{c.estimated_minutes ?? '?'} min
              </span>
              <span className="block text-[13px] font-medium truncate mt-0.5" style={{ color: '#061C2B' }}>{c.title}</span>
              {isLocked && (
                <span className="block text-[11px] text-[#55666E] mt-0.5">Finish {lockedBy} first</span>
              )}
            </span>
          </div>
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      <div className="bg-[#E9E2D2] border border-[#DCD7C6] rounded-lg border border-[#DCD7C6] shadow-sm px-4 py-5">
        <p className="text-[11px] mb-1.5" style={{ ...F_LABEL, color: '#00A8CC' }}>
        ...
        {(() => {
          ...
          return (
            <button
              ...
              className="mt-4 w-full rounded-full py-3 text-[11px] tracking-[0.16em]"
              style={{ ...F_LABEL, background: '#00D2FF', color: '#061C2B' }}
            >
...
function TierGroup({
  ...
  return (
    <div className="bg-[#E9E2D2] border border-[#DCD7C6] rounded-lg border border-[#DCD7C6] shadow-sm overflow-hidden" style={accent ? { borderTop: `3px solid ${accent}` } : undefined}>
      <button type="button" onClick={() => setOpen(!open)} className="w-full flex items-center justify-between gap-2 px-4 py-3.5 text-left">
        <span className="min-w-0">
          ...
          {open && <span className="block text-[11px] text-[#55666E] mt-1 normal-case tracking-normal" style={{ fontFamily: 'inherit' }}>{sub}</span>}
        </span>
        <span className="shrink-0 inline-flex items-center gap-2">
          <span className="text-[11px]" style={{ ...F_LABEL, color: complete ? '#06D6A0' : done > 0 ? '#00A8CC' : '#55666E' }}>
...
function CoachQuizSection({
      ...
      <div className="space-y-3">
        ...
        >
          {result.passed
            ...
            : <BookOpen size={36} strokeWidth={1.75} className="mx-auto mb-1 text-[#10263B]" />
          }
          <p className="text-2xl font-bold" style={{ color: result.passed ? '#0A7C5D' : '#10263B' }}>
            {result.score}%
          </p>
          <p
            className="text-sm font-semibold mt-1"
            style={{ color: result.passed ? '#0A7C5D' : '#10263B' }}
          >
        ...
        <div className="space-y-2">
          {quizzes.map((q, idx) => {
            ...
            return (
              <div
                key={q.id}
                className={`bg-[#F7F9FA] rounded-lg border p-4 shadow-sm ${
                  r?.gotIt ? 'border-emerald-200' : 'border-red-200'
                }`}
              >
                <p className="text-[11px] font-mono text-[#55666E] mb-1">
                  Q{idx + 1} {r?.gotIt ? '· ✓' : '· ✗'}
                </p>
                <p className="text-sm text-[#10263B]">{q.question}</p>
        ...
        {!result.passed && (
          <button
            type="button"
            onClick={retake}
            className="w-full py-3 text-white text-sm font-semibold rounded-[5px]"
  ...
  return (
    <div className="space-y-3">
      <div className="bg-[#E9E2D2] border border-[#DCD7C6] rounded-lg border border-[#DCD7C6] p-4 text-center">
      ...
      <div className="space-y-3">
        {quizzes.map((q, idx) => (
          <div key={q.id} className="bg-[#E9E2D2] border border-[#DCD7C6] rounded-lg border border-[#DCD7C6] shadow-sm p-3">
            <p className="text-[11px] font-mono text-[#55666E] mb-1">Q{idx + 1}</p>
            <p className="text-sm text-[#10263B] leading-relaxed">{q.question}</p>
      ...
      <button
        ...
        className="w-full py-3 text-white text-sm font-semibold rounded-[5px] disabled:opacity-50"
        style={{ background: BRAND.colors.navy }}
      >
        {submitting
          ? 'Scoring…'
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      {coach.portal_can_manage_boards === true && coach.academy_id && (
        <details className="bg-[#E9E2D2] border border-[#DCD7C6] rounded-lg shadow-sm overflow-hidden border border-[#DCD7C6]">
          <summary className="cursor-pointer list-none px-4 py-3.5 flex items-center justify-between">
            ...
            <span className="text-[11px] text-[#55666E]">add · edit · rent</span>
          </summary>
          <div className="p-3 border-t border-[#DCD7C6]">
            {/* El manager es dark-theme: necesita fondo tinta o queda blanco sobre blanco. */}
            <div className="rounded-[5px] p-3" style={{ background: '#0A2532' }}>
      ...
      <details className="bg-[#E9E2D2] border border-[#DCD7C6] rounded-lg shadow-sm overflow-hidden" style={{ border: '1.5px solid rgba(255,107,107,.55)' }}>
        <summary className="cursor-pointer list-none p-4 flex items-center gap-3">
          <span className="w-10 h-10 rounded-[5px] flex items-center justify-center shrink-0" style={{ background: 'rgba(255,107,107,.12)' }}>
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block text-[11px] text-[#55666E] leading-snug">Numbers, hospital, protocol</span>
          </span>
          <span className="text-[11px] shrink-0" style={{ ...F_LABEL, color: hasEmergency ? '#00A8CC' : '#FF6B6B' }}>{hasEmergency ? 'View' : 'Not set'}</span>
        </summary>
        <div className="px-4 pb-4 space-y-2.5 border-t border-[#DCD7C6] pt-3">
          {hasEmergency ? (
            <>
...
function PlanTab({
  ...
  if (selectedCampId) {
    if (loading) {
      return (
        <div className="text-center py-16">
          <ClipboardList size={36} strokeWidth={1.75} className="animate-pulse mx-auto mb-2 text-[var(--tss-cyan,#00D2FF)]" />
          <p className="text-[#55666E] text-sm">Loading planner…</p>
        </div>
      );
    }
    if (!planData) {
      return (
        <div className="text-center py-16">
          <p className="text-sm text-[#55666E]">Couldn&apos;t load this service.</p>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between gap-2 px-1">
          ...
          {planData.readOnly ? (
            ...
            <div className="inline-flex rounded-lg border border-[#DCD7C6] overflow-hidden">
                ...
                <button
                  ...
                  className={`px-3 py-1 text-[10px] uppercase tracking-wider font-semibold transition-colors ${
                    planView === v
                      ...
                      : 'bg-[#F7F9FA] text-gray-500 hover:text-gray-800'
                  }`}
            ...
            {unclosed.map((u, i) => (
              ...
              >
                <span className="min-w-0 flex items-baseline gap-[8px]">
                  <span className="shrink-0 text-[11px]" style={{ ...F_MONO, color: '#10263B' }}>
                          ...
                          <div key={s.id} style={{ borderLeft: '3px solid #00D2FF', ...(i > 0 ? { borderTop: `1px solid ${HAIR}` } : {}) }}>
                            <div className="flex items-stretch">
                              ...
                              <button
                                ...
                                className="shrink-0 px-[13px] text-[15px] hover:bg-[rgba(255,209,102,.25)] transition-colors"
                            ...
                            {trOpen === s.id && (
                              <div className="px-[13px] py-[10px]" style={{ borderTop: `1px dashed ${HAIR}`, background: 'rgba(255,209,102,.08)' }}>
                                {!trInfo ? (
                                  <p className="text-[11px] text-[#55666E]" style={F_MONO}>Loading…</p>
                                ) : !trInfo.ok ? (
                                  ...
                                  <>
                                    ...
                                    <p className="text-[11px] mb-[8px] text-[#55666E]" style={F_MONO}>
                                    ...
                                    <div className="flex items-center gap-[8px] flex-wrap">
                                      ...
                                      {d?.needed && (
                                        <button type="button" disabled={trBusy} onClick={() => saveTransport(s.id, false)}
                                          className="px-[8px] py-[8px] text-[11px] text-[#55666E] disabled:opacity-40" style={F_MONO}>
                                          Cancel request
                                        </button>
                                      )}
                                    </div>
      ...
      {past.length > 0 && (
        <details className="group">
          <summary className="cursor-pointer list-none flex items-end justify-between pb-[5px] mb-[8px]" style={{ borderBottom: `1px solid ${HAIR}` }}>
            <span className="flex items-center gap-1.5 text-[11px] text-[#55666E]" style={F_MONO}>
          ...
          <div style={{ border: `1px solid ${HAIR}`, background: '#fff' }}>
            {past.map((s: any, i: number) => (
              ...
              >
                ...
                <span className="shrink-0 text-[11px] text-[#55666E]" style={F_MONO}>
  ...
  return (
    <div style={{ borderTop: `1px dashed ${HAIRLINE}` }}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full px-[13px] py-[8px] flex items-center gap-[8px] hover:bg-[#EDF3F5]/30 transition-colors"
      >
        ...
        {roster.length > 6 && (
          <span className="text-[11px] text-[#55666E]" style={MONO}>+{roster.length - 6}</span>
        )}
        <span className="ml-auto shrink-0 text-[11px] inline-flex items-center gap-1" style={{ ...MONO, color: '#00A8CC' }}>
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-[11px] text-[#55666E]" style={MONO}>
                    {belt?.en ?? '—'}
                ...
                {r.safety_flag && (
                  <span title="Medical / safety notes — read the profile" className="shrink-0 inline-flex">
                    <ShieldAlert size={14} style={{ color: '#10263B' }} />
                  </span>
                )}
                {!r.waiver_signed && (
                  <span className="shrink-0 text-[11px] px-1.5 py-0.5 rounded-full font-semibold" style={{ background: 'rgba(255,107,107,.12)', color: '#c2410c' }}>
                    NO WAIVER
                  </span>
                )}
                <ChevronRight size={13} className="shrink-0 text-[#DCD7C6]" />
              </Link>
            );
          })}
        </div>
...
function RatingTab({ stats, onBack }: { stats: any; onBack?: () => void }) {
  return (
    <div className="space-y-4 pb-4">
      ...
      <div className="rounded-lg px-4 py-5" style={{ background: '#0A2532' }}>
        <p className="text-[11px] font-mono uppercase tracking-wider text-[var(--tss-cyan,#00D2FF)] mb-1">Your Rating</p>
        <h2 className="text-lg font-bold text-white" style={{ fontFamily: 'var(--font-heading)' }}>From your students</h2>
      </div>

      {stats.ratingsCount > 0 ? (
        <div className="rounded-lg p-6 text-center" style={{ background: '#0A2532' }}>
