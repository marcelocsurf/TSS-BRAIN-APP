    ...
    <div className="space-y-4">
      ...
      <div className="bg-[#E9E2D2] rounded-lg border border-[#DCD7C6] shadow-sm p-4">
        <div className="flex items-center justify-between gap-2">
          <button type="button" onClick={() => setWeekStart(shiftWeek(weekStart, -1))} className="px-3 py-1.5 rounded-lg border border-[#DCD7C6] text-sm">←</button>
          <p className="font-bold text-[15px]" style={{ color: INK }}>Semana {weekLabel}</p>
          <button type="button" onClick={() => setWeekStart(shiftWeek(weekStart, 1))} className="px-3 py-1.5 rounded-lg border border-[#DCD7C6] text-sm">→</button>
        </div>
        <div className="grid grid-cols-3 gap-2 mt-3 text-center">
          <div className="rounded-[5px] p-2.5" style={{ background: 'rgba(6,214,160,.1)' }}>
          ...
          <div className="rounded-[5px] p-2.5" style={{ background: 'rgba(255,209,102,.18)' }}>
          ...
          <div className="rounded-[5px] p-2.5 bg-[#F7F9FA]">
            <p className="text-[9px] font-mono uppercase tracking-wider text-[#55666E]">Ya emitido</p>
            <p className="text-[17px] font-bold text-[#55666E]">{money(totals.paid)}</p>
          </div>
        </div>
      </div>
      ...
      {data && data.duplicates.length > 0 && (
        <div className="rounded-lg p-3.5" style={{ background: 'rgba(255,209,102,.14)' }}>
      ...
      {data === null && !msg && <p className="text-sm text-[#55666E] text-center py-8">Cargando semana…</p>}
      {data && data.people.length === 0 && <p className="text-sm text-[#55666E] text-center py-8">Nadie dictó servicios esta semana.</p>}
...
function PersonCard({ p, weekStart, weekEnd, pending, start, reload }: {
  ...
  return (
    <div className="bg-[#E9E2D2] rounded-lg border border-[#DCD7C6] shadow-sm p-4">
      <div className="flex items-center justify-between gap-2">
        <div>
          <p className="font-bold text-[15px]" style={{ color: INK }}>{p.name}{p.cert ? <span className="text-[#55666E] font-normal text-[12px]"> · {p.cert}</span> : null}</p>
          <p className="text-[11px] text-[#55666E]">{p.roles.join(' + ')} · {p.days.length} día(s)</p>
        </div>
        <div className="text-right">
          ...
          {p.paid_cents > 0 && <p className="text-[10px] text-[#55666E]">emitido {money(p.paid_cents)}</p>}
        </div>
      </div>
        ...
        {p.days.map((d) => (
          <div key={d.session_id} className="py-1.5 flex items-center justify-between gap-2 text-[12px]">
            <span className="min-w-0 truncate" style={{ color: INK }}>
              <span className="font-mono text-[10px] text-[#55666E] mr-1.5">{fmtDay(d.date)}</span>
              {d.service}
              <span className="text-[#55666E]"> · {d.students} alumno{d.students === 1 ? '' : 's'}</span>
              {d.role !== 'coach' && (
      ...
      <div className="flex flex-wrap gap-2 mt-3">
        ...
        <button type="button"
          ...
          className="rounded-full px-3 py-2.5 text-[11px] border border-[#DCD7C6] text-[#55666E]">
          Historial
        </button>
      </div>
      ...
      {history !== null && (
        <div className="mt-2 rounded-[5px] bg-[#F7F9FA] p-3">
          <p className="text-[9px] font-mono uppercase tracking-wider text-[#55666E] mb-1">Pagos emitidos</p>
          {history.length === 0 ? <p className="text-[11px] text-[#55666E]">Sin pagos registrados aún.</p>
            : history.map((h: any) => (
              <p key={h.id} className="text-[11px] text-[#55666E]">
