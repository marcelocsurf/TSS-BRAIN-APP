...
export function DataRightsCard({ studentId, canAnonymize, consents }: Props) {
  ...
  return (
    <div className="bg-[#E9E2D2] rounded-lg border border-[#DCD7C6] border border-[#DCD7C6] p-5">
      ...
      {consents.anonymized_at ? (
        <p className="text-sm text-[#55666E]">Ficha anonimizada el {d(consents.anonymized_at)}. Los datos identificativos y de salud fueron borrados a pedido del titular.</p>
      ) : (
        <>
          <div className="divide-y divide-gray-50">
            {rows.map(([k, v, ok]) => (
              <div key={k} className="flex items-center justify-between gap-3 py-1.5 text-[12.5px]">
                <span className="text-[#55666E]">{k}</span>
            ...
            {consents.guardian_name && (
              <div className="flex items-center justify-between gap-3 py-1.5 text-[12.5px]">
                <span className="text-[#55666E]">Firmó por el menor</span>
                <span className="text-[#10263B]">{consents.guardian_name}</span>
              </div>
            )}
          </div>

          {canAnonymize && (
            <div className="mt-4 pt-3 border-t border-[#DCD7C6]">
              {!open ? (
                <button type="button" onClick={() => setOpen(true)} className="text-[12px] text-[#55666E] hover:text-red-600 inline-flex items-center gap-1.5">
                  <Trash2 size={13} /> El alumno pidió borrar sus datos
                </button>
              ) : (
                <div className="space-y-2">
                  <p className="text-[12px] text-[#55666E] leading-snug">
                  ...
                  <div className="flex gap-2">
                    <button type="button" disabled={busy || reason.trim().length < 5}
                      ...
                      className="px-4 py-2 rounded-[5px] text-[12px] font-semibold bg-red-600 text-white disabled:opacity-40">
                      {busy ? 'Anonimizando…' : 'Anonimizar ficha'}
                    </button>
                    <button type="button" onClick={() => { setOpen(false); setReason(''); }} className="px-4 py-2 rounded-[5px] text-[12px] text-[#55666E]">Cancelar</button>
                  </div>
                </div>
              )}
            </div>
