...
export function SetCoachPasswordCard({ coachId }: { coachId: string }) {
  ...
  return (
    <div className="bg-[#E9E2D2] rounded-lg border border-[#DCD7C6] shadow-sm p-5 space-y-3">
      ...
      <p className="text-xs text-[#55666E] leading-relaxed">
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 px-3 py-2.5 border border-[#DCD7C6] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[var(--tss-cyan,#5AC3E7)]"
        />
        <button type="button" onClick={suggest} className="px-3 py-2.5 text-xs font-semibold rounded-lg border border-[#DCD7C6] text-[#55666E] hover:bg-[#F7F9FA] shrink-0">
          Suggest
        </button>
      </div>
      ...
      {done ? (
        <div className="rounded-[5px] bg-green-50 border border-green-200 p-3">
          ...
          <div className="flex items-center gap-2 mt-2">
            <code className="flex-1 text-sm bg-[#F7F9FA] border border-green-200 rounded-lg px-3 py-2 font-mono text-[var(--tss-navy)]">{password.trim()}</code>
