...
// ═══ Núcleo del reporte de EXPERIENCIA del camp, SIN auth ═══
// Lo usan /reports/experiencia (dashboard) y el portal del host (Kat).
import type { createAdminClient } from '@/lib/supabase/admin';
import { toElSalvadorDate } from '@/lib/utils/tz';
import { DIM_COLS, type ExperienceCol } from '@/lib/survey/experience-questions';
import type { ReportScopeLite } from './satisfaction-core';
...
export const emptyDims = (): Record<ExperienceCol, { avg: number | null; n: number }> =>
  Object.fromEntries(DIM_COLS.map((c) => [c, { avg: null, n: 0 }])) as any;

export function dimAverages(rows: any[]): Record<ExperienceCol, { avg: number | null; n: number }> {
  const out = emptyDims();
...
export function npsOf(rows: any[]): { score: number | null; promoters: number; passives: number; detractors: number; n: number } {
...
export type ExperienceCore = Pick<ExperienceReport, 'totals' | 'camps' | 'responses' | 'alerts'>;

export async function experienceReportCore(admin: Admin, scope: ReportScopeLite, from: string, to: string): Promise<{ error?: string } | ExperienceCore> {
