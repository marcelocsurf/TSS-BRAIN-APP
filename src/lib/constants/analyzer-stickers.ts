// ═══ STICKERS DEL ANALIZADOR ═══
// GENERADO por scripts/sync-stickers.mjs — no editar a mano.
// Para sumar un sticker: dejar el archivo en la carpeta STICKERS del
// escritorio y correr `node scripts/sync-stickers.mjs`.
//
// Son STICKERS, no transparencias: se ven PEGADOS encima del video. Sin
// nombres ni categorías — el coach los ve en una grilla y elige.

export type StickerDef = { id: string; src: string; ratio: number };

export const STICKERS: StickerDef[] = [
  { id: 'circulo-infinito', src: '/stickers/circulo-infinito.png', ratio: 1.7964 },
  { id: 'etapas-de-la-ola', src: '/stickers/etapas-de-la-ola.png', ratio: 0.36 },
  { id: 'ola', src: '/stickers/ola.png', ratio: 0.3333 },
  { id: 'pie', src: '/stickers/pie.png', ratio: 1.5 },
  { id: 'tabla', src: '/stickers/tabla.png', ratio: 3.9474 },
  { id: 'tres-circulos-color', src: '/stickers/tres-circulos-color.png', ratio: 0.6667 },
  { id: 'tres-circulos', src: '/stickers/tres-circulos.png', ratio: 0.83 },
  { id: 'triangulo', src: '/stickers/triangulo.png', ratio: 1 },
  { id: 'carita-feliz', src: '/stickers/carita-feliz.svg', ratio: 1 },
  { id: 'dragon', src: '/stickers/dragon.svg', ratio: 0.6774 },
  { id: 'llamas', src: '/stickers/llamas.svg', ratio: 0.3541 },
  { id: 'pizza-mitad', src: '/stickers/pizza-mitad.svg', ratio: 0.5755 },
  { id: 'pizza-porcion', src: '/stickers/pizza-porcion.svg', ratio: 1.303 },
  { id: 'sweet-spot', src: '/stickers/sweet-spot.svg', ratio: 1 },
];
