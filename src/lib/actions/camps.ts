...
import { seedPlansFromTemplate } from '@/lib/camps/seed-plan';
...
export async function listCampsInRange(startDate: string, endDate: string) {
  ...
  let query = supabase
    .from('camp_instances')
    .select(
      'id, camp_name, start_date, end_date, status, modality, scheduled_time, capacity_override, template_id, head_coach_id, coach_id, is_holding, is_test, camp_templates(template_name, level_name, includes_course_key, service_kind, capacity_max, duration_days, session_duration_minutes, card_color, accent_color), head_coach:head_coach_id(display_name, max_belt_permission), coaches:coach_id(display_name, role), head_coach_id, head_coach_status, camp_participants(id, enrollment_status, payment_status, student_id, amount_cents)'
    )
...
export async function createCampInstance(input: {
  ...
  // La receta vive en src/lib/camps/seed-plan.ts (2026-09-25): la misma que
  // usa el cambio de plantilla de un camp ya creado.
  if (createdSessions && createdSessions.length > 0 && input.student_ids.length > 0) {
    await seedPlansFromTemplate(supabase as any, {
      instanceId: instance.id,
      templateDayIds: days.map((d) => d.id),
      sessions: createdSessions.map((cs: any) => ({ id: cs.id, template_day_id: cs.template_day_id ?? null })),
...
export async function getCampDetail(campId: string) {
  ...
  const { data: instance, error: instanceErr } = await supabase
    .from('camp_instances')
    .select('*, camp_templates(*), coaches:coach_id(display_name), head_coach:head_coach_id(id, display_name, max_belt_permission)')
    .eq('id', campId)
    .single();
...
// ═══ Camp "por confirmar" de la academia (Marcelo 2026-09-11) ═══
export async function getHoldingCamp(): Promise<{ id: string; camp_name: string } | null> {
  const me = await getCurrentCoach();
  ...
  const { data } = await admin
    .from('camp_instances')
    .select('id, camp_name')
    .eq('academy_id', me.academy_id).eq('is_holding', true).neq('status', 'cancelled')
    .maybeSingle();
  return data ?? null;
}
